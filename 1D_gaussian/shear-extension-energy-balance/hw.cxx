...
private:
  Field3D T, K;
  Field3D U, kx;              // mean flow and radial wavenumber
  Field3D chi_T, chi;
  Field3D S, Rxy;             // diagnostics: shear and Reynolds stress
  ...
  // NEW: energy-consistent coupling strength
  BoutReal alpha_RS;
...
protected:
  int init(bool restarting) override {
    ...
    // Shear-suppressed diffusivity parameters
    OPTION(opt, chi1, 10.0);
    OPTION(opt, S_star, 1.0);

    // Reynolds stress + eikonal parameters
    OPTION(opt, C_R, 0.1);
    ...
    // NEW: energy-consistent transfer term coefficient
    // Adds -alpha_RS * Rxy * S to ddt(K)
    OPTION(opt, alpha_RS, 1.0);
  ...
  int rhs(BoutReal /*time*/) override {
    ...
    // 5) K equation (with safe pow and energy-consistent coupling)
    Field3D K_safe = K;
    ...
    // Energy-consistent transfer from turbulence to mean flow:
    // mean-flow energy source is +Rxy*S, so turbulence must have a sink -alpha_RS*Rxy*S
